# 家族スケジュールアプリ 開発タスク
#
# - run/stop/build/test/jar : API (Java)。devcontainer 内で実行する
# - mobile-*                : macOS ホストで実行する（RN はホストで動かす方針。コンテナ内では動かない）
#
# `make` だけ打つとタスク一覧を表示する。

.DEFAULT_GOAL := help
.PHONY: help run stop build test jar mobile-install mobile-start mobile-clean
...
test: ## テスト実行（要: db コンテナ稼働）
	cd api && ./mvnw -B test

jar: ## 本番形態の実行可能 jar を作成
